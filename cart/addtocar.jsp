            ...
            try 
            {
                ...
                {   
					...
					if(session.getAttribute("username") != null ){
                    	...
                    	con.createStatement().execute("USE album");
                    String sql = "";
				
					sql = "select `buy_count` FROM `car` WHERE `mem_id`='"+getuser+"'AND `procar_id`='"+pro_id+"' ";
					...
					while(cc.next())
					{
						...
						sql = "UPDATE `car` SET `porcar`='"+ number3 +"' WHERE `mem_id`='"+getuser+"'AND `procar_id`='"+pro_id+"' ";
					}
					if(ismuilt){

					}
					else {
						sql = "INSERT `car`(procar_id, mem_id, buy_count) "+"VALUES('"+pro_id+"','"+getuser+"','"+count+"')";
                    	int no = con.createStatement().executeUpdate(sql); 
					}
					con.close();
                    out.print("<meta http-equiv='refresh' content='0;url=car.jsp'>");
					}
					else{
			 			con.close();//結束資料庫連結
